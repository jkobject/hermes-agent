...
def _preview_message_content(
    content: Any,
    max_chars: int = _MESSAGE_CONTENT_PREVIEW_CHARS,
) -> tuple[Any, Optional[int]]:
    """Return content unchanged, or a bounded recoverable preview."""
    ...
    prefix_chars = max(0, max_chars - len(marker))
    return content[:prefix_chars] + marker[:max_chars], len(content)
...
def _shape_message(
    ...
    """Slim a message row while bounding oversized historical content.

    Callers may request a tighter bound. With no explicit bound, the local
    12k safety cap prevents one stored tool result from rebuilding the active
    model context. Canonical content remains in SessionDB.
    """
    raw_content = m.get("content")
    effective_limit = (
        _MESSAGE_CONTENT_PREVIEW_CHARS
        if max_content_len is None
        else min(max_content_len, _MESSAGE_CONTENT_PREVIEW_CHARS)
    )
    content, original_chars = _preview_message_content(
        raw_content, max_chars=effective_limit
    )
    truncated = original_chars is not None
    ...
    if truncated:
        ...
        # Keep the local compatibility key while exposing upstream's canonical
        # metadata name to newer callers.
        entry["full_content_chars"] = original_chars
