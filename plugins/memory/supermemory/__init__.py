...
def _resolve_base_url(config_value: Any = "", legacy_config_value: Any = "") -> str:
    """Resolve the API URL, preserving the pre-``base_url`` local config.

    Precedence: ``base_url`` config > legacy ``api_url`` config >
    ``SUPERMEMORY_BASE_URL`` > legacy ``SUPERMEMORY_API_URL`` > default.

    Supports self-hosted Supermemory servers (e.g. http://localhost:6767).
    """
    raw = (
        str(config_value or "").strip()
        or str(legacy_config_value or "").strip()
        or os.environ.get("SUPERMEMORY_BASE_URL", "").strip()
        or os.environ.get("SUPERMEMORY_API_URL", "").strip()
    )
...
def _load_supermemory_config(hermes_home: str) -> dict:
    ...
    config["api_url"] = str(config.get("api_url", "") or "").strip()
...
def _probe_supermemory_connection(api_key: str, hermes_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(hermes_home)
    base_url = _resolve_base_url(config["base_url"], config["api_url"])
    status = {
        "ok": False,
        "error": "",
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._base_url = _resolve_base_url(
            self._config["base_url"], self._config["api_url"]
        )
